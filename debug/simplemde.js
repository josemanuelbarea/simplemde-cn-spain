...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.span.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.renderer.span(this.output(cap[2] || cap[1]));
      continue;
    }

    // noshow
    if (cap = this.rules.noshow.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.renderer.noshow(this.output(cap[1]));
      continue;
    }
...
Renderer.prototype.span = function(text) {
  return '<span class="span_widget">' + text + '</span>';
};

Renderer.prototype.noshow = function(text) {
  return '';
};
...
TextRenderer.prototype.span =
TextRenderer.prototype.noshow =
...
var marked = require("marked-cn-spain/lib/marked.js");
...
/**
 * Action for draw an span class.
 */
function drawSpan(editor) {
	var cm = editor.codemirror;
	var stat = getState(cm);
	var options = editor.options;
	_replaceSelection(cm, stat.span, options.insertTexts.span);
}

/**
 * Action for draw an attribute html.
 */
function drawAttribute(editor) {
	var cm = editor.codemirror;
	var stat = getState(cm);
	var options = editor.options;
	_replaceSelection(cm, stat.attribute, options.insertTexts.attribute);
}
...
SimpleMDE.drawSpan = drawSpan;
SimpleMDE.drawAttribute = drawAttribute;
...
SimpleMDE.prototype.drawSpan = function() {
	drawSpan(this);
};
SimpleMDE.prototype.drawAttribute = function() {
	drawAttribute(this);
};
